...
class KnightsAndKnaves extends Table
{
	function __construct( )
	{
        ...
        $this->idcards = self::getNew( "module.common.deck" );
        $this->idcards->init( "idcard" );      
        $this->qcards = self::getNew( "module.common.deck" );
        $this->qcards->init( "qcard" );      
        // $this->kcards = self::getNew( "module.common.deck" );
    ...
    */
    ...
    {    
        ...
        //                   3 = keep cards
        // FOR US:
        // Maybe we have states for asking questions, answering questions, and making guesses. 
        ...
        // Create cards FIXXXXX!!!!
        $qcards = array ();
        foreach ( $this->colors as $color_id => $color ) {
            // spade, heart, diamond, club
            for ($value = 2; $value <= 14; $value ++) {
                //  2, 3, 4, ... K, A
                $qcards [] = array ('type' => $color_id,'type_arg' => $value,'nbr' => 1 );
            }
        }
        // Create cards FIXXXXX!!!!
        $idcards = array ();
        foreach ( $this->colors as $color_id => $color ) {
            ...
            for ($value = 2; $value <= 14; $value ++) {
                //  2, 3, 4, ... K, A
                $idcards [] = array ('type' => $color_id,'type_arg' => $value,'nbr' => 1 );
            }
        }
        
        # Define the two decks, referencing the sql tables
        $this->idcards->createCards( $idcards, 'iddeck' );
        $this->qcards->createCards( $qcards, 'qdeck' );

        // Shuffle deck
        $this->idcards->shuffle('iddeck');
    ...
    */
    protected function getAllDatas()
    {
        ...
        // We need something like this, but I don't know what we need to do to get it
        // $result['identity'] = $this->idcards->getCardsInLocation( 'hand', $current_player_id );
...
//////////// 
    ...
    /*
        ...
        (note: each method below must match an input method in knightsandknaves.action.php)
    */
    ...
    */
    ...
    function playCard( $card_id ) {
      ...
      // Check that this is the player's turn and that it is a "possible action" at this game state (see states.inc.php)
        ...
        // In hearts, they set the current trick color using self::setGameStateValue( 'trickColor', $currentCard['type'] ); if this is the first card played that trick.
        
        // Notify all players about the card played
        // self::notifyAllPlayers( 'cardPlayed', clienttranslate( '${player_name} plays ${card_id}' ), array(
        //   'player_id' => $player_id,
